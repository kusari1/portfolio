...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["image"])) {
    ...
    if (!in_array($fileType, $allowedTypes)) {
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
            // 「プレースホルダ（仮の値）」 で、後からPHPのコードで実際の値を入れるためのもの。
            $stmt = $conn->prepare("INSERT INTO images (image_name, public_flg) VALUES ($customName, ?)");
...
<form action="work30.php" method="post" enctype="multipart/form-data">
    ...
    <br>
    画像名: <input type="text" name="custom_name" placeholder="画像の名前を入力"> <!-- 画像名を任意で入力 -->
    <br>
