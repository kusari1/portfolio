...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["toggle_id"])) {
    ...
    function img_switch(){
      // 画像の公開・非公開を更新するSQL文
      $stmt = $dbconect->prepare("UPDATE images SET public_flg = ?, update_date = NOW() WHERE image_id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["image"])) {
    ...
    if (!preg_match('/^[a-zA-Z0-9]+$/', $customName)) {
        die("<p style='color: red;'>エラー: 画像名は半角英数字のみ使用できます。</p>");
    }
...
<div class="image-container">
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            $statusimg = $publicFlg ? "publicimg" : "privateimg";//公開・非公開の背景色
            ...
            echo "<img src='uploads/$imageName' alt='$imageName' style='width:50%;'>"; // 画像表示
            echo "<p>$imageName</p>";
